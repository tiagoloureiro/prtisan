...
export async function pruneTrainArtifacts(input: {
  ...
  if (await directoryExists(input.runner, trainsRoot)) {
    ...
    await input.runner.run("find", [trainsRoot, "-path", "*/logs/*", "-type", "f", "-mtime", ttl, "-delete"]);
    await input.runner.run("find", [trainsRoot, "-path", "*/logs/*", "-type", "f", "-size", maxLogSize, "-delete"]);
  }
  ...
  for (const subdir of ["logs", "patches"]) {
    ...
    await input.runner.run("find", [path, "-type", "f", "-mtime", ttl, "-delete"]);
    await input.runner.run("find", [path, "-type", "f", "-size", maxLogSize, "-delete"]);
  }
...
async function directoryExists(runner: CommandRunner, path: string): Promise<boolean> {
