...
export async function executeCreatePrs(
  ...
  let state = (await pathExists(existingStatePath))
    ? reconcileTrainState(
        await loadTrainState(input.cwd, trainId),
        input.config,
        graph,
        branchPlan,
        now
  ...
  for (const layer of graph.layers) {
    deps.log?.(
      `Implementing layer: ${layer.map((issueNumber) => `#${issueNumber}`).join(", ")}`
    );

    await mapLimit(
      layer,
      input.config.concurrency.implement,
      async (issueNumber) => {
        ...
        if (node.externalOpenBlockers.length > 0) {
          ...
          await githubMutate(() =>
            deps.github.createIssueComment(
              input.config.repo,
              issueNumber,
              `Agent train blocked: ${message}`
            )
          );
          return;
        }

        const blockerBranchMissing = await firstMissingBlockerBranch(
          record.blockers,
          state,
          deps.git
        );
        ...
        if (record.syntheticBase) {
          const syntheticBranch = record.syntheticBase;
          try {
            await gitMutate(() =>
              deps.git.createSyntheticBaseBranch({
                ...
                issueNumber,
                syntheticBranch,
                blockerBranches: record.blockers.map(
                  (blocker) => state.issues[String(blocker)]!.branch
                ),
              })
            );
        ...
        try {
          baseAnchorSha = await gitMutate(() =>
            deps.git.revParseRemoteBranch(record.baseBranch)
          );
        } catch (error) {
