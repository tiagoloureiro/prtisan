...
export function issueBranch(config: AgentTrainConfig, issue: Pick<Issue, "number" | "title">): string {
  return `${config.branchPrefix}${issue.number}-${slugify(issue.title)}`;
}

export function syntheticBaseBranch(config: AgentTrainConfig, trainId: string, issueNumber: number): string {
