...
import { buildIssueGraph, planBranches } from "../src/graph.js";
import { createTrainState, mergeMetadataIntoPrBody, reconcileTrainState } from "../src/state.js";
...
describe("PR metadata", () => {
  test("replaces prior metadata block idempotently", () => {
    const first = mergeMetadataIntoPrBody("Body", "<!-- agent-train\n{\"trainId\":\"a\"}\nagent-train -->");
    const second = mergeMetadataIntoPrBody(first, "<!-- agent-train\n{\"trainId\":\"b\"}\nagent-train -->");
  ...
  test("reconciles fresh issue data while preserving opened PR metadata", () => {
    const initialGraph = buildIssueGraph([issue({ number: 1, title: "Old title" })]);
    const initialPlan = planBranches(initialGraph, config, "train-a");
    const initialState = createTrainState("train-a", config, initialGraph, initialPlan, new Date("2026-07-23T00:00:00Z"));
    const openedState = {
      ...initialState,
      issues: {
        "1": {
    ...
    const freshGraph = buildIssueGraph([issue({ number: 1, title: "New title" })]);
